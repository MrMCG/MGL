...
MGLShader::MGLShader(ShaderType shaderType) : MGLShader(determineShdaerDefault(shaderType), shaderType) {
	std::cout << "Loading default shader: " << shaderType << std::endl;
}

MGLShader::MGLShader(std::string const& shaderSrc, ShaderType shaderType) : m_shaderType(shaderType) {
...
std::string const MGLShader::determineShdaerDefault(ShaderType shaderType) {
	switch (shaderType) {
